    ...
    protected function runJob(Job $job, $runnableRetains)
    {
        ...
        foreach ($runnableRetains as $retain) {
            ...
            foreach ($commands as $command) {
                ...
                $tmplogfile = sprintf('%s/rsnapshot.%04d_%04d.log', $logDir, $idClient, $idJob);
                // Let's save this log into {logs}/jobs
                $joblogfile = sprintf('%s/%s.rsnapshot.%04d_%04d.log', "$logDir/jobs", time(), $idClient, $idJob);
                $this->moveLogfile($tmplogfile, $joblogfile);
                ...
                // Ends successfully
                    ...
                    $commandOutput = $this->captureStatsFromLogfile($joblogfile);
                    if ($commandOutput){
