...
find .debian -type f -name "*.deb" -print0 | xargs -0 rm -rf
find .debian/usr/share/elkarbackup/public/js/dojo-release-* -name "*.uncompressed.js" -print0 | xargs -0 rm -f
...
find .debian/usr/share/elkarbackup/public/js/dojo-release-* -type f -print0 | xargs -0 chmod a-x
...
find .debian -type d -print0 | xargs -0 -I {} chmod 755 "{}"
...
(
    cd .debian/usr/share/elkarbackup || exit 1
    find . | grep -E -v 'bin|config|extra|migrations|public|src|templates|translations|var|vendor' | xargs rm -rf
)
...
# build an verify
#
if ! fakeroot dpkg-deb --build .debian "elkarbackup_${VERSION}_all.deb"; then
    echo "Error building package."
    exit 1
fi
echo Package created

lintian "elkarbackup_${VERSION}_all.deb" | tee lintian.log | grep -E '^E'
